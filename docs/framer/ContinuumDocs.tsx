...
// source-sha256: ee8a23faa7d1cd022f48fe73ba723c2f0a27c05d6ce5eaecf140a3408881279c
...
<span class="cm">#     reader, even a turn that calls no tool at all.</span></pre></div>
  ...
  <h3>Asking a person first</h3>
  <p>The gates above decide by <em>rule</em>. Some actions are legitimate but
  consequential, and the honest answer is "allow, but ask someone" — which no
  allow/deny rule can express. That is configured on the agent, not written as a
  policy resource:</p>
  <div class="code-wrapper"><button class="copy-btn" onclick="copyCode(this)">copy</button><pre><span class="cls">AgentConfig</span>(
    tool_approval={<span class="str">"send_payment"</span>, <span class="str">"*transfer*"</span>},   <span class="cm"># fnmatch, as policy resources</span>
    approval_handler=ask_a_human,               <span class="cm"># async, you supply it</span>
    approval_timeout=<span class="num">30.0</span>,
)

<span class="cm"># async def ask_a_human(req: ToolApprovalRequest) -&gt; ToolApprovalDecision</span></pre></div>
  <p><strong>This is the only gate that sees the tool's ARGUMENTS.</strong> The
  policy gate checks <code>tool:&lt;name&gt;</code> and MCP tool-trust vets a
  server's catalogue once, offline — so neither can tell
  <code>transfer(amount=5)</code> from <code>transfer(amount=5_000_000)</code>.
  The request carries the arguments, the agent name, the run id and the run's
  current data labels, because "this run has read the public web" is exactly what
  a reviewer needs before approving an outbound action.</p>
  <p><strong>No default list of risky tools.</strong> <code>delete_*</code>,
  <code>send_*</code>, <code>pay_*</code> reads as a sensible default and is not
  one: it blocks a harmless <code>send_receipt</code> while missing
  <code>wire_funds</code>, and a gate nobody configured is assumed to cover more
  than it does. You declare; the runtime enforces.</p>
  <p><strong>It fails closed on every path</strong> — a handler that raises, one
  that never answers, one that returns something other than a decision, and a
  tool declared for approval with no handler wired. That last is the
  configuration most easily mistaken for protection, so it refuses rather than
  quietly proceeding (and is warned about once at startup). A refusal comes back
  as a tool result — <code>APPROVAL DENIED: …</code> — so the model tells the
  user and the turn ends normally, the same ergonomics as a policy denial.</p>
  <p><strong>Three limits worth knowing before you ship it.</strong> A blocked
  run holds the HTTP request open while the reviewer thinks, and browsers,
  proxies and serverless platforms give up long before a person does — hence the
  30s default; a wait measured in hours needs a durable workflow or a
  refuse-and-resume UI, not a bigger timeout. Approvals are serialised rather
  than batched, so a reviewer sees one call at a time rather than "this turn
  wants to do these three things". And approvals are not remembered: a retried
  run asks again, because a cached approval would authorise an execution the
  reviewer never saw — <code>run_id</code> and <code>arguments</code> are on the
  request so you can dedupe with a policy that suits your tools.</p>
