...
**Guides** — [Setup & index](TESTING_GUIDE.md) ·
[Labels & policy](labels-and-policy.md) · [F6 memory](F6-memory.md) ·
[F3 server trust](F3-server-trust.md) · [Namespacing](namespacing.md)
...
| **Memory** | an EXTERNAL run's row IS stored, stamped with its origin, fenced on recall (F6) | `F6-memory.md` |
...
Optional switches, each documented in TESTING_GUIDE.md (the index — it links
...
theoretical; see namespacing.md.
